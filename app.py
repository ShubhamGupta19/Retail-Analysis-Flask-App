...
def uploaddata():
    if request.method == "POST":
        ...
        if householdfile and allowed_file(householdfile.filename) and transactionfile and allowed_file(transactionfile.filename) and productsfile and allowed_file(productsfile.filename):
            ...
            #      cursor.execute('''
            #      INSERT INTO PRODUCTS(PRODUCT_NUM,DEPARTMENT,COMMODITY,BRAND_TY,NATURAL_ORGANIC_FLAG) VALUES (?,?,?,?,?)
            #      ''',
            #      row.PRODUCT_NUM,
            #      row.DEPARTMENT,
            #      row.COMMODITY,
            ...
            # reader = csv.reader(codecs.iterdecode(productsfile, 'utf-8'))
